...
#실습하기 4-6
SELECT SUM(`sale`) AS `함계` FROM `Sales`;
...
#확인문제 2. 2019년 2월 5만원 이상 매출에 대한 총합과 평균을 구하시오.
SELECT
	SUM(`sale`) AS `총합`,
	AVG(`sale`) AS `평균`
FROM
...
#확인문제 3. 2020년 전체매출 가운데 최저, 최고 매출을 구하시오
SELECT 
	MIN(`sale`) AS `최저`,
	MAX(`sale`) AS `최고`
FROM 
...
#확인문제 3. 2019년 50000원이상의 매출에 대해 직원별 매출의 합이 
#				100000원 이상인 직원의 이름, 부서명,직급, 년도, 매출합을 조회하시오
#				단, 매출 합이 큰 순서부터 정렬
